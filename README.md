...
- **読み上げボイスを選択式で切り替え**: 「端末/ブラウザ内蔵ボイス」「VOICEVOX（無料の日本語キャラクターボイスエンジン、サーバー方式）」「Google Cloud TTS（自然なニューラル音声、サーバー不要）」「VOICEVOX（内蔵・Android限定・実験的）」の中から選べます。声質・キャラクター性を重視するならVOICEVOX、サーバー運用の手間なく自然な声を使いたいならGoogle Cloud TTSがおすすめです（セットアップ方法は下記）。
- **Googleアカウントで会話ログ・AIの記憶を端末間で引き継ぐ**: 利用者自身のGoogleアカウントでログインすると、会話ログと「AIが要約した、その人についての短い記憶」を、そのアカウント自身のGoogleドライブ(このアプリ専用の非公開領域)に同期します。開発者のサーバーには送信されません。連携自体は無料です(詳細は下記)。**有料プランへの加入・管理者判定もこのGoogleアカウントのメールアドレス単位で行うため、有料機能を使うには連携が必須**になりました。
...
- **有料プラン(サブスクリプション)と管理者**: 「備え付けのAI」(共有プロキシ)とVOICEVOX(端末にない読み上げボイス)は有料プラン加入者向けの機能です。Stripeでの決済リンクにより継続課金を受け付けられます。加入状況・管理者かどうかは、上記のGoogleアカウント連携で得られる**メールアドレス**単位で判定します(Googleが署名したIDトークンをサーバー側で検証するため、他人のメールアドレスを騙ることはできません)。アプリ配布者(管理者)は、自分のメールアドレスをサーバー側に登録しておくことで、課金なしで全機能を使えます。「自分のAPIキーを使う」モードと端末内蔵ボイスはプランに関係なく無料です。
...
「備え付けのAI」とVOICEVOXは有料プラン加入者向けの機能として実装されています。加入状況・管理者判定は、下記「7. Googleアカウントで会話ログ・AIの記憶を引き継ぐ」で連携するGoogleアカウントの**メールアドレス**単位で行うため、**先に7章のGoogle Cloud Console設定を済ませておく必要があります**。Stripeでの決済リンクの作成、Cloudflare Workers KVでのサブスク状態管理、管理者メールアドレスの設定などの手順は **[proxy-worker/README.md](./proxy-worker/README.md)** の「有料プラン(サブスクリプション)をセットアップする」の章にまとめています。

この設定を行わない場合でも、管理者メールアドレス(`ADMIN_EMAILS`)だけ設定しておけば、あなた自身は課金なしで全機能を使えます。他の利用者への課金導線が不要な場合は、Stripe連携そのものを省略しても構いません。
...
1. Googleアカウント連携(下記7)をした上で、有料プランに加入する(設定画面の「利用プラン」から)。管理者として設定済みのメールアドレスでログインしている場合は、加入なしでそのまま使えます。
...
### 7. Googleアカウントで会話ログ・AIの記憶を引き継ぐ

**有料プラン(サブスクリプション)を使う場合は、この章の設定が必須です**(加入・管理者判定をこのGoogleアカウントのメールアドレス単位で行うため)。有料プランを使わない(共有プロキシもVOICEVOXも使わせない)配布であれば、この章はスキップして構いません。

Web/Android両方に対応した、端末をまたいだ会話の引き継ぎ機能です。利用者が自分のGoogleアカウントでログインすると、会話ログと「AIがこれまでの会話から要約した、その人についての短い記憶」(呼び方の希望、最近の話題、大事にしていることなど)を、**そのアカウント自身のGoogleドライブ**(このアプリ専用の非公開領域=`appDataFolder`。通常のドライブ画面には一切表示されません)に保存します。別の端末で同じGoogleアカウントにログインすると、そこから会話を続けられます。

このアプリの一貫した設計方針(会話内容は端末内のみに保存し、開発者のサーバーには送らない)に合わせ、**開発者側のサーバーには一切送信・保存されません**。有料プランとは無関係に、無料で誰でも使える機能です。

#### 何が起きるか(技術的な仕組み)

- 会話を送信するたびに、バックグラウンドでドライブへ保存されます(体感速度には影響しません)。
- 起動時、ドライブ側に端末より新しいデータがあれば自動的に取り込みます。
- 8往復程度(利用者発言+AI返答で1往復)の会話が溜まるごとに、AI自身に「次回以降も覚えておくとよい要点」を短く要約させ、記憶を更新します(裏側で使う対話AIへの追加リクエストが1回増えるということです。共有プロキシの無料枠を使っている場合、通常の会話に比べて消費が少し増える点はご留意ください)。この記憶は次回以降の会話のシステムプロンプトに小さく添えられ、AIがそれとなく踏まえて話すようになります。
- Web版は簡略化のため、ログイン時に得たアクセストークン(有効期限は約1時間)のみを保持し、長期間有効なリフレッシュトークンは保存しません。アプリを開き直すなどしてトークンが切れた場合は、再度ログインボタンを押す必要があります(その間も会話自体は普通に続けられ、再ログイン後にまた同期が再開されます)。Android版はOSのアカウント管理に任せているため、この制約はありません。

#### セットアップ手順(アプリ配布者が行う作業)

Googleアカウント連携にはGoogle Cloud ConsoleでのOAuthクライアント作成が必要です。VOICEVOXやGoogle Cloud TTSで既にGoogle Cloudのプロジェクトを作っている場合は、同じプロジェクトで構いません。

1. [Google Cloud Console](https://console.cloud.google.com/)でプロジェクトを作成(または既存のものを使用)する。
2. [Google Drive APIの有効化ページ](https://console.cloud.google.com/apis/library/drive.googleapis.com)を開き、「有効にする」を押す。
3. 左メニューの「APIとサービス」→「OAuth同意画面」で同意画面を設定する。
   - 公開ステータスは**「テスト」のままで構いません**(最大100人まで、Googleの追加審査なしにすぐ使えます)。この場合、ログイン時に「Google未確認のアプリ」という警告画面が出ますが、「詳細」→「(アプリ名)に移動(安全ではないページ)」から進めば正常にログインできます。100人を超える利用者に配布したい場合は、公開ステータスを「本番環境」にし、Googleの基本審査(プライバシーポリシーURLの提出などが必要。今回使うスコープは「機密性の低いスコープ」扱いのため、追加のセキュリティ審査(CASA)までは不要です)を受けてください。
   - スコープの追加で `.../auth/drive.appdata` と `.../auth/userinfo.email`、`openid` を追加する。
4. 「認証情報」→「認証情報を作成」→「OAuthクライアントID」で、以下の**2つ**を作成する。
   - **種類「ウェブ アプリケーション」**: 承認済みのリダイレクトURIに、Web公開先のURL + `/oauth/google` を追加する(例: `https://<あなたのユーザー名>.github.io/<リポジトリ名>/oauth/google`)。作成後に表示される**クライアントID**と**クライアントシークレット**を、`lib/config.ts` の `GOOGLE_WEB_CLIENT_ID` / `GOOGLE_WEB_CLIENT_SECRET` に設定する。
   - **種類「Android」**: パッケージ名に `com.example.kokorotalk`(`app.json`の`android.package`と同じ値)を、SHA-1証明書フィンガープリントに **`59:A1:9F:ED:D4:38:23:D2:65:DD:E0:78:21:A3:43:08:66:C3:FC:B8`** を入力する(このリポジトリに同梱している、CIビルド用に固定化したdebug鍵`android-debug-keystore/debug.keystore`のフィンガープリントです。詳細は`.github/workflows/build-apk.yml`の該当ステップ参照)。**このクライアントIDはコード上のどこにも設定不要です**(Google Play servicesがビルドの署名から自動的に判定します。作成しておくだけで機能します)。
5. `lib/config.ts` を書き換えたら、アプリをビルドし直してください。

#### 正直な注記

Web版のログイン(`lib/googleAuth.web.ts`)・Android版のGoogleドライブ同期(`lib/googleDriveSync.ts`)ともに、この開発環境では実際のGoogle Cloud Consoleクライアントを使った実地でのログイン確認はできていません(Google Cloud Consoleでの認証情報作成はアプリ配布者=あなたご自身の作業のため)。実装はGoogleの公式ドキュメント・OAuthの仕様に基づいていますが、実際にログインしてみてエラーが出た場合は、エラーメッセージ(および可能であればブラウザの開発者ツールのコンソール)を共有していただければ修正します。
...
この方式では、公開URLに**リポジトリ名がそのまま含まれます**。リポジトリ名がアプリの内容を連想させるものだと困る場合は、下記「Web版のURLをリポジトリ名と無関係にする」を参照してください。

### Web版のURLをリポジトリ名と無関係にする(Cloudflare Pages、任意)

GitHub Pagesの `username.github.io/リポジトリ名` という形式が気になる場合、**Cloudflare Pages**に切り替えることで、リポジトリ名と無関係な独自のURLで公開できます(無料。`proxy-worker`で使っているCloudflareアカウントをそのまま使えます)。

**正直な前提として**、この方法でもGitHubのリポジトリ自体は非公開にはできません(個人アカウントでは、GitHub Pagesを使う限りリポジトリを公開にする必要があります)。あくまで「URLを見ただけでは中身を連想できない」ようにするものであり、リポジトリを直接見つけられた場合はソースコード・READMEから内容が分かってしまう点はご理解ください。
...
app/                  画面 (expo-router)
  ...
  oauth/google.tsx     Google OAuthログインのリダイレクト先ページ(Web版のみ)
components/           UI部品
  ChatBubble.tsx        チャットの吹き出し
  VoiceButton.tsx       マイクボタン
lib/                  ロジック(UIに依存しない部分)
  types.ts              型定義
  config.ts               共有プロキシの接続情報・Google連携のクライアントID等
  ...
  googleAuth.ts            Googleサインイン(Android/iOS。ネイティブSDK使用)
  googleAuth.web.ts        Googleサインイン(Web。OAuth PKCE自前実装)
  googleDriveSync.ts       Googleドライブ(appDataFolder)への会話ログ・記憶の保存/取得
  memory.ts                AIによる会話からの「記憶」要約の生成・更新
  sync.ts                  起動時の取り込み・送信ごとのアップロードなど同期の司令塔
...
proxy-worker/         対話AIの共有プロキシ(Cloudflare Workers)。開発者のAPIキーをここに集約
  ...
  src/googleIdToken.js     Google IDトークン(JWT)の署名検証ロジック
  test-jwt-verify.mjs      ↑の検証ロジックの単体テスト(自己署名JWTで確認、Google通信なし)
