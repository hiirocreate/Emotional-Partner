...
- **Googleアカウントで会話ログ・AIの記憶を端末間で引き継ぐ(任意)**: 利用者自身のGoogleアカウントでログインすると、会話ログと「AIが要約した、その人についての短い記憶」を、そのアカウント自身のGoogleドライブ(このアプリ専用の非公開領域)に同期します。開発者のサーバーには送信されません。有料プランに関係なく無料で使えます(詳細は下記)。
...
### 7. Googleアカウントで会話ログ・AIの記憶を引き継ぐ(任意)
